...
namespace dotth {
	...
	public:
		...
		void set_eye(const vector3& v);
		void set_up(const vector3& v);
		void set_at(const vector3& v);
		
		void set_pers(const float& z_near, const float& z_far, const float& width, const float& height, const float& fov);
		
		void set_pers_near(const float& v);
		void set_pers_far(const float& v);
		void set_aspect(const float& w, const float& h);
		void set_fov(const float& v);
		...
		void set_ortho_near(const float& v);
		void set_ortho_far(const float& v);
		void set_ltrb(const float& l, const float& t, const float& r, const float& b);
		void set_ortho1px(const float& _1px);
	};
};
