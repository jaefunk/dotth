...
void dotth::render::command::primitive_orthographic::draw(void)
{
	_shader.bind();
...
void dotth::render::command::primitive_perspective::draw(void)
{
	_shader.bind();

	auto ot = glGetUniformLocation(_shader.program(), "view_pers");
	glUniformMatrix4fv(ot, 1, GL_FALSE, camera::instance()->view_pers());
	auto m = glGetUniformLocation(_shader.program(), "model");
	glUniformMatrix4fv(m, 1, GL_FALSE, _model);
	auto vtx = glGetAttribLocation(_shader.program(), "vertex");
	glEnableVertexAttribArray(vtx);
