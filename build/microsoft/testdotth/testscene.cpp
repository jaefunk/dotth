...
void testscene::init(const json& data) 
{
	...
	parent->trans->scl(0.5f, 0.5f, 0.5f);
	attach(parent);

	auto child = std::make_shared<dotth::rectangle>();
	child->init();
	child->set_color({ 1.f, 0.f, 1.f, 1.f });
	child->trans->scl(0.5f, 0.5f, 0.5f);
	parent->attach(child);

	auto child2 = std::make_shared<dotth::rectangle>();
	child2->init();
	child2->set_color({ 1.f, 1.f, 1.f, 1.f });
	child2->trans->scl(0.5f, 0.5f, 0.5f);
	child->attach(child2);
}
